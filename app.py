...
async def lolucantseeidkok(ctx):
 ...
 # embed.add_field(name="<a:spy_cyan_crown:894150074567909406>Help Menu<a:spy_cyan_crown:894150074567909406>", value='[Invite](https://dsc.gg/spy-sec)\n[Support](https://discord.gg/XDzUVexw4d)')
  embed.add_field(name="<:slash:920014228197359656> Massunban", value='```"Unbans all banned users, aliases - unbanall"```')
  embed.add_field(name="<:slash:920014228197359656> Lockserver", value='```"Revokes dangerous perms from each role, aliases - lockroles"```')
  embed.add_field(name="<:slash:920014228197359656> Cleanchannels", value='```"Deletes channel with similar names, aliases - cc"```') 
  await ctx.reply(embed=embed)
...
#   idk = ""
    ...
    embed.add_field(name="<a:spy_success:919998568041971782>SUCESS", value=f'```('Unbanning {} members....'.format(len(banlist))```')
    await ctx.reply(embed=embed)
